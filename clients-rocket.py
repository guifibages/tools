...
class Antenna:
    ...
    def status(self):
        self.status = self.http_client.get("https://%s/status.cgi"
                                           % self.host, verify=False).json()
        self.links = self.http_client.get("https://%s/sta.cgi"
                                          % self.host, verify=False).json()
    ...
    def __str__(self):
        ...
        for l in sorted(self.links, key=lambda l: l['airmax']['priority']):
            ...
            r = r + ("%14s %15s  %8s %7.0f  %3.0f %6.2f %6.1fKm\n"
                     % (l['name'], l['lastip'], priority,
                        l['airmax']['quality'], l['ccq'], l['signal'],
                        l['distance']/1000))
        return r
